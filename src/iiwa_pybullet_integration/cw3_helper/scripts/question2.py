...
class cw3_question2_script:
    ...
    def set_pose(self, axis, i):
        ...
        y_offset = 0; x_offset = 0; z_offset_0 = 0.3063972977; z_offset_1 = 0.348930734; z_offset_2 = 0.3113972977
        
        if(axis == 'y'):
            self.pose.pose.position.y = np.mean(self.object_ty) + y_offset
            print('For y-axis')
        elif(axis == 'x'):
            self.pose.pose.position.x = np.mean(self.object_tx) + x_offset
            print('For x-axis')
        elif(axis == 'z'):
            if(i == 0):
                self.pose.pose.position.z = np.mean(self.object_tz) + z_offset_0
            elif(i == 1):
                self.pose.pose.position.z = np.mean(self.object_tz) + z_offset_1
            elif(i == 2):
                self.pose.pose.position.z = np.mean(self.object_tz) + z_offset_2
            else:
    ...
    def get_object(self, i):
        ...
        self.object_tx = []; self.object_ty = []; self.object_tz = []
        self.object_ox = []; self.object_oy = []; self.object_oz = []; self.object_ow = []
        ...
        for itr in range(15):
            ...
            while not rospy.is_shutdown():
                try:
                    ...
                    self.object_tx.append(self.object_pose.transform.translation.x)
                    self.object_ty.append(self.object_pose.transform.translation.y)
                    self.object_tz.append(self.object_pose.transform.translation.z)

                    self.object_ox.append(self.object_pose.transform.rotation.x)
                    self.object_oy.append(self.object_pose.transform.rotation.y)
                    self.object_oz.append(self.object_pose.transform.rotation.z)
                    self.object_ow.append(self.object_pose.transform.rotation.w)

                    break
                except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException):
                    continue
        
        object_angles = [np.mean(self.object_ox), np.mean(self.object_oy), np.mean(self.object_oz), np.mean(self.object_ow)]
        ...
        if(i == 0):
            ...
            if((self.c_ff >= 1 and self.c_mf >= 1 and self.c_rf >= 1 and self.c_th >= 1) or (self.c_ff >= 1 and self.c_mf >= 1)):
                print('Good Contact!')
                self.grasp('open')
                self.set_z_home()
            else:
