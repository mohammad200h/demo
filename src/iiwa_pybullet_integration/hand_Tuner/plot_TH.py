...
def get_starting_pos_and_command(active_joints_info,generate=True,stable=True):
    ...
    if generate:
        ...
        file_name = "th_stable.csv" if stable else "th_unstable.csv"
        path = "./"+file_name
        csvfile = open(path)
...
def try_pid(generate=True,stable=True,log=True,plot=False,figure_num=0):
...
if __name__ == "__main__":
    ...
    for i in range(1000):
        try_pid(generate=False,stable=True,log=False,plot=True,figure_num=i+1)
