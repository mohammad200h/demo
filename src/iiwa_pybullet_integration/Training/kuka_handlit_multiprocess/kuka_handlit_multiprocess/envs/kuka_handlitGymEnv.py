...
class PerformanceMetricVisulization():
  ...
  def plot_performance(self,performance,average_performance,title,path=None):
    ...
    labels = ['Palm', 'FF', 'MF', 'RF', 'TH']
    Best = [performance["palm"],performance["ff"],performance["mf"],performance["rf"],performance["th"]]
    Average = [average_performance["palm"],average_performance["ff"],average_performance["mf"],average_performance["rf"],average_performance["th"]]
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(x - width/2, Best, width)
    rects2 = ax.bar(x + width/2, Average, width)
  ...
  def plot_finger_touch(self,percentag,path=None):
    ...
    percentage=[percentag["ff"],percentag["mf"],percentag["rf"],percentag["th"]]
    ...
    ax.set_title("Percentage of fingers touching the target during an episode")
    ax.set_xticks(x, labels)
    ax.legend()
    ax.set_ylim(0, 1)
  ...
  def plot_table_collision(self,collision,path=None):
    ...
    rects1 = ax.bar(x, percentage, width,align='center')

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Percentage')
    ax.set_title("Collision With Table")
...
class PerformanceMetric():
  def __init__(self,obs_object,record_performance=False):
    ...
    self.perofrmance_log = {
      ...
      "best_performance":None, # best episode 
      "best_terminal_state_performance":None,
      "average_best_performance":None, # average performance accross all episdoes
      "average_termination_performance":None
    }
  ...
  def performance_during_episode(self,step):
    
    if self.record_performance:

      performance_is_better = False
      state = self.obs_obj.obs_distAndOrn_relativeToPalmAndFingers(worldFrame=False)

      if self.first_run:
        performance_is_better = True
        self.first_run = False
      else:
        performance_is_better = self.Is_Perofrmance_better_than_last_step(state[:5])
      
      if performance_is_better:
        self.episdoe_perofrmance_log["best_performance_during_episode"]["dist"] = {
              "palm":state[0],
              "ff"  :state[1],
              "mf"  :state[2],
              "rf"  :state[3],
  ...
  def performance_at_end_of_episode(self,finger_touch_dic,collision_with_table_flag):
    ...
    if self.record_performance and not self.first_run:
      ...
      self.episdoe_perofrmance_log["performance_at_end_of_episode"] = {
            ...
            "touch":{
                  "ff":finger_touch_dic["distal_FF"],
                  "mf":finger_touch_dic["distal_MF"],
                  "rf":finger_touch_dic["distal_RF"],
                  "th":finger_touch_dic["thdistal"]
            },
            "collision":collision_with_table_flag
        }

      self.perofrmance_log["episdoes"].append(self.episdoe_perofrmance_log) 
  ...
  def calculate_performacne_accross_episdoes(self):
    ...
    average_performance["performance_at_end_of_episode"]["collision"] = self.claculate_percentage_of_collision_with_table()
    
    
    self.perofrmance_log["best_performance"]                 =   self.find_best_performance_during_episode()
    self.perofrmance_log["best_terminal_state_performance"]  =   self.find_best_performance_at_end_episode()
  ...
  def Is_Perofrmance_better_than_last_step(self,state):
    ...
    """
    current_state = {}
    better_performance_counter = 0

    # print("Is_Perofrmance_better_than_last_step::state",state)

    current_state["palm"] =state[0] 
    current_state["ff"]   =state[1]
    current_state["mf"]   =state[2]
    current_state["rf"]   =state[3]
    current_state["th"]   =state[4]

    # print("Is_Perofrmance_better_than_last_step::current_state:: ",current_state)

    for key,value in self.episdoe_perofrmance_log["best_performance_during_episode"]["dist"].items():
      # print("key,value:: ",key,value)
      if current_state[key]< value:
        better_performance_counter +=1
    
    if better_performance_counter >2:
      return True
  ...
  def calculate_ave_performance(self,performance_dic_key):
    ...
    #sum
    for e in self.perofrmance_log["episdoes"]:
     for key in ave.keys():
       ave[key] += e[performance_dic_key]["dist"][key]

    # average
    for key in ave.keys():
      ave[key] /=len(self.perofrmance_log["episdoes"])
  ...
  def calculate_percentage_of_fingers_touching(self):
    ...
    for e in self.perofrmance_log["episdoes"]:
     for key in percentage.keys():
       if e["performance_at_end_of_episode"]["touch"][key]:
         percentage[key] +=1 
  
    # percentage
    for key in percentage.keys():
      percentage[key] /= len(self.perofrmance_log["episdoes"]) 
      percentage[key] *=100

    return percentage

  def claculate_percentage_of_collision_with_table(self):
    percentage = 0

    for e in self.perofrmance_log["episdoes"]:
      if e["performance_at_end_of_episode"]["collision"]:
        percentage +=1
    
    percentage /= len(self.perofrmance_log["episdoes"])
    percentage *= 100

    return percentage


  def find_best_performance_during_episode(self):
    pereformance = {
          "dist":{
            "palm":float('inf'),
            "ff"  :float('inf'),
    ...
    for e in self.perofrmance_log["episdoes"]:
      for key in pereformance["dist"].keys():
        ...
        if pereformance["dist"][key] > e["best_performance_during_episode"]["dist"][key]:
          performance_counter +=1
      
      if performance_counter>3:
        pereformance = e["best_performance_during_episode"]
        performance_counter = 0
  ...
  def find_best_performance_at_end_episode(self):
    ...
    pereformance =  {
      "dist":{
          ...
          "collision":False  #percentage
    }
    
    dist_performance_counter = 0
    flag_performance_counter = 0
    for e in self.perofrmance_log["episdoes"]:
      for key in pereformance["dist"].keys():
        if (pereformance["dist"][key] > e["performance_at_end_of_episode"]["dist"][key]):
          dist_performance_counter +=1

      for key in pereformance["touch"].keys():
        if (pereformance["touch"][key] < e["performance_at_end_of_episode"]["touch"][key]):
          flag_performance_counter +=1

      if flag_performance_counter>3 or dist_performance_counter>3:
        pereformance = e["performance_at_end_of_episode"]
        dist_performance_counter = 0
        flag_performance_counter = 0
...
class Kuka_HandlitGymEnv(gym.Env):
  ...
  def __init__(self,render=False, timeStep = 1./1000.,obs_mode = "kuka_world",
    ...
    self.perfromanceMeteric = PerformanceMetric(self.obs_obj,record_performance)
  ...
  def step(self,action):
    ...
    self.perfromanceMeteric.performance_during_episode(self._envStepCounter)
    #################################################
  ...
  def _termination(self):
    ...
    if (self.grasp_scuesss_flag or 
      ...
      self.perfromanceMeteric.performance_at_end_of_episode(fingers_tip_touch,collision_with_table_flag)
      ################################################
      return True
