...
class TactileArray {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.data.forEach((val) => {
      bufferOffset = Tactile.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.data[i] = Tactile.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.data.forEach((val) => {
      length += Tactile.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.data !== undefined) {
      ...
      for (let i = 0; i < resolved.data.length; ++i) {
        resolved.data[i] = Tactile.Resolve(msg.data[i]);
      }
    }
    else {
      resolved.data = []
